...
class Input{
    ...
    test(){
        ...
        var inputVal = this.testInput.val();

        if (this.maxRange !== null && this.minRange !== null){
            if(inputVal >= this.maxRange || inputVal <= this.minRange){
                successObject={
                    result:false,
                    error:'range'
                }
	...
	notes: should create an element and position it directly beneath the input
		...
		*/
	showError( message ){
		var positionInput = this.testInput.position();
        var heightInput = this.testInput.height();
        var parentInput = this.testInput.parent();
        this.errorMessage = $("<div>",{
			css: {
				top: positionInput.top + heightInput,
				left: positionInput.left
			},
			"class": "inputError",
            text: message
		});
		parentInput.append(this.errorMessage);
	}
	/*
